...
const OneGameItems = ({ gameResult, myParticipantIdx }) => {
  const myInfo = gameResult?.info?.participants[myParticipantIdx];
  return (
    <div className="col m-0 p-0">
      <div style={{ height: "100%" }} className="container-fluid d-flex flex-column align-items-center justify-content-center p-0">
        <div className="row row-cols-4 gap-1 m-0 mt-1">
          <Item item={myInfo?.item0} />
          <Item item={myInfo?.item1} />
          <Item item={myInfo?.item2} />
          <Item item={myInfo?.item6} />
        </div>
        <div className="row row-cols-4 gap-1 m-0 my-1">
